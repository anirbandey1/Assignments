...
/** ::------ ASSIGNMENT DETAILS ------::
 * Assignment 7:  Thread, Synchronizations & Shared Memory (Resubmit)
 *
 * Problem Definition:
...
/** ::------ COMPILATION COMMAND ------::
    gcc 17_7_ReSubmit.c -o 17_7_ReSubmit -Wall -lpthread
 *
 * ::------ EXECUTION SEQUENCE ------::
   Open a Linux terminal and run:   ./17_7_ReSubmit <number_of_pairs>
...
/* custom structure to store the values and the pairs - created to allocate shared memory using shmget */
...
char **_endptr = NULL;               /* pointer for parsing commandline arguments */
...
void die(const char *message, const char *action)
{
    ...
    if (_endptr != NULL)
        free(_endptr); /* free the memory allocated during commandline parsing */
    exit(EXIT_FAILURE);
}
...
{
    float X, Y, A, E;
    int n, *curr, *shm_id = (int *)args;
    shm_17 *shared_memory = NULL;

    /* attach an address to the shared memory */
    if ((shared_memory = (shm_17 *)shmat(*shm_id, NULL, 0)) == (void *)-1)
        die("thread1 shmat", "check shm_id");

    n = shared_memory->n;
    curr = &shared_memory->row; /* curr will act as an alias for shared_memory->row */
    ...
    while (*curr < n)
    {
        ...
        /* if current row is the last row, don't continue anymore */
        if (*curr == n - 1)
            break;

        /* signal the main process that next row can be generated */
        pthread_barrier_wait(&barrier); /* barrier synchronization */
    }

    /* detach shared memory */
    if (shmdt(shared_memory) != 0)
        die("thread1 shmdt", NULL);
...
void *thread2_function(void *args)
{
    ...
    /* attach an address to the shared memory */
    if ((shared_memory = (shm_17 *)shmat(*shm_id, NULL, 0)) == (void *)-1)
        die("thread2 shmat", "check shm_id");
...
void *thread3_function(void *args)
{
    ...
    /* attach an address to the shared memory */
    if ((shared_memory = (shm_17 *)shmat(*shm_id, NULL, 0)) == (void *)-1)
        die("thread3 shmat", "check shm_id");
...
void print_table(shm_17 *shared_memory, unsigned int n)
{
    ...
    for (i = 0; i < n; i++)
    {
        if (shared_memory[i].pair[0] == 10 && shared_memory[i].pair[1] == 10) /* if both X and Y are 10 apply different spacing */
        {
...
{
    int n, error, shmid, assign_no = 17;
    ...
    {
        printf("Number of pairs cannot be negative or zero!\n");
        free(endptr);
        exit(EXIT_FAILURE);
    }
    ...
    else if ((shmid = shmget(key, n * sizeof *shm, 0666 | IPC_CREAT)) == -1)
        die("shmget", "make sure that there is no existing SHM segment with same ID but different size");
    /* attach an address to the shared memory */
    else if ((shm = (shm_17 *)shmat(shmid, NULL, 0)) == (void *)-1)
        die("shmat", NULL);
    ...
    shm->n = n;    /* store the value of n in shared memory */
    shm->row = -1; /* store the current row pointer in shared memory */

    printf("\nShared memory allocated successfully. Creating threads...\n");

    /* create the threads */
    if ((error = pthread_create(&thread1, NULL, &thread1_function, (void *)&shmid)) != 0)
        printf("\nThread 1 cannot be created: [%s]", strerror(error));
    else if ((error = pthread_create(&thread2, NULL, &thread2_function, (void *)&shmid)) != 0)
        printf("\nThread 2 cannot be created: [%s]", strerror(error));
    else if ((error = pthread_create(&thread3, NULL, &thread3_function, (void *)&shmid)) != 0)
        printf("\nThread 3 cannot be created: [%s]", strerror(error));

    printf("Threads created successfully. Waiting for them to finish execution...\n");

    while (shm->row < n) /* for n elements */
    {
        shm->row += 1; /* increment the current row number */

        /* generate a new random pair */
        shm[shm->row].pair[0] = rand() % (MAX_VALUE + 1);
        shm[shm->row].pair[1] = rand() % (MAX_VALUE + 1);

        /* signal the threads to start computation */
        pthread_barrier_wait(&barrier); /* barrier synchronization */
    ...
    print_value_dict(); /* print which thread computes which of the values */
